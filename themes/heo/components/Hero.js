  ...
  return (
    <div className="h-[165px] select-none xl:h-20 flex flex-col justify-between xl:space-y-0 xl:flex-row w-28 lg:w-48 xl:w-full xl:flex-nowrap xl:space-x-3">
      ...
      <Link
        ...
        className="group relative overflow-hidden bg-gradient-to-r from-[#e4c4b9] to-[#e9decb] hidden h-20 xl:flex justify-start items-center text-white rounded-xl xl:hover:w-1/2 xl:w-1/3 transition-all duration-500 ease-in"
      >
          ...
          return (
            <Link href={`${siteConfig('SUB_PATH', '')}/${p?.slug}`} key={index}>
              <div className="cursor-pointer h-[164px] group relative flex flex-col w-52 xl:w-full overflow-hidden shadow bg-white dark:bg-black dark:text-white rounded-xl">
                ...
                {/* hover 悬浮的 ‘荐’ 字 */}
                <div className="opacity-0 group-hover:opacity-100 -translate-x-4 group-hover:translate-x-0 duration-200 transition-all absolute -top-2 -left-2 bg-[#b96a6f] dark:bg-yellow-600  text-white rounded-xl overflow-hidden pr-2 pb-2 pl-4 pt-4 text-xs">
                  荐
                </div>
              </div>
            </Link>
  ...
  return (
    ...
    >
      <div
        ...
        } shadow transition-all duration-200 today-card h-full bg-[#b96a6f] rounded-xl relative overflow-hidden flex items-end`}
      >
        <div
          id="today-card-info"
